    ...
    try {
      ...
      if (doc.exists) {
        ...
        setState(() {
          ...
          _hoursController.text = data['hours'] ?? '8AM-6PM';
          _fssaiController.text = data['fssai'] ?? '';
          _ownerController.text = data['ownerName'] ?? '';
          _notificationsEnabled = data['notificationsEnabled'] ?? true;
          _autoAcceptRequests = data['autoAcceptRequests'] ?? false;
          _deliveryRadius = data['deliveryRadius']?.toString() ?? '5';
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      await _firestore.collection('restaurants').doc(user.uid).update({
        ...
        'autoAcceptRequests': _autoAcceptRequests,
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildSwitchTile(
                                ...
                                subtitle:
                                    'Receive alerts about donation requests',
                              ),
                              _buildSwitchTile(
                                value: _autoAcceptRequests,
                                onChanged:
                                    (value) => setState(
                                      () => _autoAcceptRequests = value,
                                    ),
                                title: 'Auto-Accept Trusted NGOs',
                                subtitle:
                                    'Automatically accept requests from verified organizations',
                              ),
                              const SizedBox(height: 16),
                              Padding(
